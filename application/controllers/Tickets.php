...
if( $precio_ticket->id_preciotickets==5|| $precio_ticket->id_preciotickets==6){

			                $pdf->SetFont('Courier', '', 11);
 $pdf->Cell(60, 4,'', 0, 1, 'C'); 
                    $pdf->MultiCell(60, 3, "Cupon ".utf8_decode($precio_ticket->tipo)." con 10% de descuento en alimentos y bebidas dentro del Hotel Montetaxco valido para ".$personas . " persona" . (($personas > 1) ? "s" : "")." el dia ".date("d/m/Y")." con el folio de venta Teleferico ".  $detalleventa->folio_venta."--".$data['hri']." NO ACUMULABLE", 1, 'C');
                   }
            }
            $pdf->AutoPrint();
            $pdf->Output();
